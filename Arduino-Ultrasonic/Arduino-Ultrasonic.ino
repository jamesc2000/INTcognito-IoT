...
#define statusPin 5 // high if door is open, low if door is closed, NOTE: This will be 5V, esp32
                    // needs 3.3V double check first, do not plug directly
...
void pushDistance(int dist) {
  ...
  const int tolerance = 500; // Any spikes larger than a 500cm difference from
                             // previous will trigger filter

  while (dist - last10Distances[9] > tolerance) {
    // If a spike is detected, get median of last good distance and the new
    // measurement and store it as the new dist. Repeat this until the spike is
